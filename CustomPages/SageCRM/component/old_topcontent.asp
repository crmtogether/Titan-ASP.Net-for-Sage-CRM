...
<% if  ( (_version=="6.0") || (_version=="6.1") || (_version=="6.2")) { %>
    <LINK REL="stylesheet" HREF="/<%=sInstallName%>/eware.css">
    <%} else {%>
    <LINK REL="stylesheet" HREF="/<%=sInstallName%>/Themes/color1.css">
    <% } %>
</HEAD>
  <BODY CLASS=TOPBODY VLINK=#003B72 LINK=#003B72 LEFTMARGIN=0 TOPMARGIN=0>
    <TABLE CELLPADDING=0 CELLSPACING=0 BORDER=0 HEIGHT=99% WIDTH=100%>
      <TR>
        <TD ALIGN=LEFT WIDTH=50>
<% if  ( (_version=="6.0") || (_version=="6.1") || (_version=="6.2")) { %>
          <IMG SRC="/<%=sInstallName%>/img/Icons/<%=icon%>" HSPACE=0 BORDER=0 ALIGN=TOP>
    <%} else {%>
