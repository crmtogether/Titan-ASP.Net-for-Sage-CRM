...
try
{
 ...
 // Response.Write(EntityWhere+"<br /><br />");
  ...
  //fix the list for ordering as CRM 'sometimes 'puts in 2 'eWState0ssCaselistOrderByColName' tags which breaks things
  var matchstring="eWState0"+ListBlock+"OrderByColName";
  var strRegExPattern = ''+matchstring+'';
  var res_match=xresponse.match(new RegExp(strRegExPattern,'gi'));
  if ((res_match!=null)&&(res_match.length==4))
  {
    xresponse=xresponse.replace(new RegExp(strRegExPattern,'i'), "crmfix1"); 
    xresponse=xresponse.replace(new RegExp(strRegExPattern,'i'), "crmfix2");   
  }
